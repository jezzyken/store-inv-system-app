...
router.get("/sales-report", async (req, res) => {
  try {
    ...
    const salesReport = sales.map((sale) => {
      const saleProducts = saleItems.filter(
    ...
    const summary = {
      ...
      totalRevenue: salesReport.reduce(
      ...
      averageProfit:
        salesReport.length > 0
          ? salesReport.reduce((sum, sale) => sum + sale.profit, 0) /
            salesReport.length
          : 0,
    };
  ...
  try {
    ...
    const inventoryReport = products.map((product) => {
      ...
      const totalVariantStock = product.variants.reduce(
        (sum, variant) => sum + (variant.stocks || 0),
        0
      );
    ...
    const summary = {
      ...
      totalStock: inventoryReport.reduce(
        (sum, product) => sum + product.totalStock,
        0
      ),
      lowStockItems: inventoryReport.filter(
      ...
      inventoryValue: inventoryReport.reduce((sum, product) => {
        ...
        const variantValue = product.variants.reduce(
          (vSum, variant) => vSum + variant.stocks * variant.cost,
          0
        );
...
router.get('/debtor-report', async (req, res) => {
  try {
    ...
    console.log(error)
    res.status(500).json({
      message: 'Error generating debtor report',
      error: error.message
    });
  }
});

router.get('/debtor-transactions/:id', async (req, res) => {
  try {
    ...
    const payments = await DebtorPayment.find({
      debtor: req.params.id
    }).lean();

    res.json({ creditSales, payments });
